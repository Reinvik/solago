              ...
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '12px', fontWeight: 800, color: '#334155', display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    💡 ¿El cliente desea vuelto en $USD pagando en Bolívares?
                  </span>
                  <button
                    type="button"
                    onClick={() => setShowTargetCalc(!showTargetCalc)}
                    style={{
                      background: showTargetCalc ? '#e0f2fe' : '#ffffff',
                  ...
                  >
                    {showTargetCalc ? '✖ Ocultar Calculadora' : '🧮 Abrir Calculadora'}
                  </button>
                </div>
                ...
                {showTargetCalc && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '10px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '10px' }}>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={targetPaidBillUSD}
                          onChange={(e) => setTargetPaidBillUSD(e.target.value)}
                          style={{ width: '100%', boxSizing: 'border-box', padding: '7px 10px', fontSize: '13px', fontWeight: 800, borderRadius: '8px', border: '1px solid #cbd5e1', background: '#fff' }}
                        />
                        <div style={{ display: 'flex', gap: '4px', marginTop: '6px', flexWrap: 'wrap' }}>
                          {[5, 10, 20, 50, 100].map(b => (
                            <button
                              key={b}
                              type="button"
                              onClick={() => setTargetPaidBillUSD(String(b))}
                              style={{
                                padding: '3px 7px',
                                fontSize: '10.5px',
                                fontWeight: 800,
                                borderRadius: '6px',
                                background: targetPaidBillUSD === String(b) ? '#059669' : '#dcfce7',
                                color: targetPaidBillUSD === String(b) ? '#fff' : '#15803d',
                      ...
                      {/* Monto Entregado en Bolívares */}
                      <div>
                        <label style={{ fontSize: '11px', fontWeight: 800, color: '#334155', display: 'block', marginBottom: '4px' }}>
                          🇻🇪 o Monto en Bolívares (Bs.) Entregado:
                        </label>
                        <input
                          type="number"
                          step="any"
                          ...
                          value={targetPaidBillVES}
                          onChange={(e) => setTargetPaidBillVES(e.target.value)}
                          style={{ width: '100%', boxSizing: 'border-box', padding: '7px 10px', fontSize: '13px', fontWeight: 800, borderRadius: '8px', border: '1px solid #cbd5e1', background: '#fff' }}
                        />
                      </div>
                    ...
                    {/* TARJETA DE RESULTADOS DE VUELTO MULTIDIVISA */}
                    {targetChangeCalcResult.totalPaidUSD > 0 && (
                      <div style={{
                        background: '#ffffff',
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '6px' }}>
                          <span style={{ fontSize: '11.5px', color: '#64748b', fontWeight: 700 }}>
                            Cobro Venta: <strong>${targetChangeCalcResult.cartTotalUSD.toFixed(2)} USD</strong> (Bs. {targetChangeCalcResult.cartTotalVES.toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })})
                          </span>
                          <span style={{ fontSize: '11.5px', color: '#0369a1', fontWeight: 800 }}>
                            Total Ingresado: <strong>${targetChangeCalcResult.totalPaidUSD.toFixed(2)} USD</strong>
                          </span>
                        </div>

                        {/* CASO SOBRAPAGO: VUELTO EN USD, BOLÍVARES O MIXTO */}
                        {targetChangeCalcResult.isOverpaid && (
                          <div style={{ background: '#f0fdf4', border: '1.5px solid #86efac', borderRadius: '10px', padding: '12px' }}>
                            <div style={{ marginBottom: '10px' }}>
                              <span style={{ fontSize: '11px', color: '#166534', fontWeight: 900, textTransform: 'uppercase', display: 'block' }}>
                                💵 VUELTO TOTAL A ENTREGAR AL CLIENTE:
                              </span>
                              <div style={{ fontSize: '20px', fontWeight: 900, color: '#15803d' }}>
                                ${targetChangeCalcResult.totalChangeUSD.toFixed(2)} USD <span style={{ fontSize: '14px', color: '#166534' }}>= Bs. {targetChangeCalcResult.totalChangeVES.toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                              </div>
                            </div>

                            <div style={{ fontSize: '11px', fontWeight: 800, color: '#166534', marginBottom: '8px' }}>
                              🎯 Formas de entregar el vuelto:
                            </div>

                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '8px', marginBottom: '8px' }}>
                              {/* 1. Vuelto 100% Dólares */}
                              <div style={{ background: '#ffffff', border: '1px solid #86efac', borderRadius: '8px', padding: '8px 10px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                                <div>
                                  <span style={{ fontSize: '10.5px', fontWeight: 800, color: '#047857', display: 'block' }}>💵 Vuelto 100% en $USD:</span>
                                  <span style={{ fontSize: '14px', fontWeight: 900, color: '#15803d' }}>${targetChangeCalcResult.totalChangeUSD.toFixed(2)} USD</span>
                                </div>
                                <button
                                  type="button"
                                  onClick={() => applyTargetChangeCalculation('usd')}
                                  style={{ marginTop: '6px', background: '#059669', color: '#fff', border: 'none', padding: '5px 8px', borderRadius: '6px', fontSize: '10.5px', fontWeight: 900, cursor: 'pointer' }}
                                >
                                  ⚡ Dar $USD
                                </button>
                              </div>

                              {/* 2. Vuelto 100% Bolívares */}
                              <div style={{ background: '#ffffff', border: '1px solid #86efac', borderRadius: '8px', padding: '8px 10px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                                <div>
                                  <span style={{ fontSize: '10.5px', fontWeight: 800, color: '#047857', display: 'block' }}>🇻🇪 Vuelto 100% en Bolívares:</span>
                                  <span style={{ fontSize: '13px', fontWeight: 900, color: '#15803d' }}>Bs. {targetChangeCalcResult.totalChangeVES.toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                </div>
                                <button
                                  type="button"
                                  onClick={() => applyTargetChangeCalculation('ves')}
                                  style={{ marginTop: '6px', background: '#0284c7', color: '#fff', border: 'none', padding: '5px 8px', borderRadius: '6px', fontSize: '10.5px', fontWeight: 900, cursor: 'pointer' }}
                                >
                                  ⚡ Dar Bolívares
                                </button>
                              </div>
                            ...
                            {/* 3. Vuelto Mixto (Dólares billete + Bolívares restante) */}
                            <div style={{ background: '#ffffff', border: '1px solid #7dd3fc', borderRadius: '8px', padding: '8px 10px' }}>
                              <span style={{ fontSize: '10.5px', fontWeight: 800, color: '#0369a1', display: 'block', marginBottom: '4px' }}>
                                🔀 Vuelto Mixto (Billetes $USD + Bolívares en vuelto):
                              </span>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                                <label style={{ fontSize: '10.5px', color: '#475569' }}>Billetes $USD a entregar:</label>
                                <input
                                  type="number"
                                  step="any"
                                  placeholder="Ej: 2"
                                  value={targetChangeInput}
                                  onChange={(e) => setTargetChangeInput(e.target.value)}
                                  style={{ width: '70px', padding: '4px 6px', fontSize: '12px', fontWeight: 800, borderRadius: '6px', border: '1px solid #cbd5e1' }}
                                />
                                {targetChangeCalcResult.wantedUSDChange > 0 && (
                                  <span style={{ fontSize: '11px', fontWeight: 800, color: '#0284c7' }}>
                                    + Vuelto en Bs: <strong>Bs. {targetChangeCalcResult.vesChangeDelivered.toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>
                                  </span>
                                )}
                                <button
                                  type="button"
                                  onClick={() => applyTargetChangeCalculation('mixto')}
                                  style={{ marginLeft: 'auto', background: '#0284c7', color: '#fff', border: 'none', padding: '5px 10px', borderRadius: '6px', fontSize: '10.5px', fontWeight: 900, cursor: 'pointer' }}
                                >
                                  ⚡ Cargar Mixto
                                </button>
                              </div>
                        ...
                        {/* CASO FALTANTE POR PAGAR */}
                        {targetChangeCalcResult.isUnderpaid && (
                          <div style={{ background: '#fef2f2', border: '1.5px solid #fca5a5', borderRadius: '10px', padding: '10px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                            <div>
                              <span style={{ fontSize: '10.5px', color: '#991b1b', fontWeight: 900, textTransform: 'uppercase', display: 'block' }}>
                                ⚠️ FALTANTE POR PAGAR EN BOLÍVARES / PAGO MÓVIL:
                              </span>
                              <div style={{ fontSize: '18px', fontWeight: 900, color: '#dc2626' }}>
                                Bs. {targetChangeCalcResult.remainingVESToPay.toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                <span style={{ fontSize: '12px', color: '#7f1d1d', fontWeight: 700, marginLeft: '6px' }}>
                                  (${targetChangeCalcResult.remainingUSDToPay.toFixed(2)} USD)
                                </span>
                              </div>
                            </div>
                            <button
                              type="button"
                              onClick={() => applyTargetChangeCalculation('underpaid')}
                              style={{ background: '#dc2626', color: '#ffffff', border: 'none', padding: '7px 12px', fontSize: '11.5px', fontWeight: 900, borderRadius: '8px', cursor: 'pointer' }}
                            >
                              ⚡ Cargar Restante
                            </button>
                          </div>
